...
<style>
  ...
  .btn-filter { background: linear-gradient(90deg, #f9a8d4, #f472b6); }
...
{{-- 🔍 Filter Status --}}
<div class="card" style="display:flex;gap:10px;justify-content:center;align-items:center;margin-bottom:20px;">
  <a href="{{ route('pengadaan.index', ['filter' => 'all']) }}" 
     class="btn btn-filter {{ $filter=='all'?'shadow-lg':'' }}">📋 Semua</a>
  <a href="{{ route('pengadaan.index', ['filter' => 'proses']) }}" 
     class="btn btn-filter {{ $filter=='proses'?'shadow-lg':'' }}">🔄 Proses</a>
  <a href="{{ route('pengadaan.index', ['filter' => 'selesai']) }}" 
     class="btn btn-filter {{ $filter=='selesai'?'shadow-lg':'' }}">📦 Selesai</a>
</div>
